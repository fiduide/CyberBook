...
if($_SESSION['group'] != 'admin' && $_SESSION['group'] != 'membre'){ //Si session est vide alors c'est un visiteur sans connection
    ...
    //On envoit d'abord un mail afin de prévenir à l'ancienne adresse mail
    ...
    if (!empty($_POST['email']) && !empty($_POST['tel']) && !empty($_POST['mdp'])){
        ...
        $to = $req['email']; //Mettre le destinataire ou l'on veut recevoir le message
        $subject="Confirmation de modification de votre compte";
        $headers="From : ".$from;
        ...
        <section class="id_Compte">
            ...
            <div style="margin-top: 1em; font-size: 20px;">
                <form onsubmit="return verifFormModifCompteUtilisateur();" action="user_Compte.php" method="POST">
                    ...
                    <label><B>Confirmation du mot de passe</B> : </label><input id="mdpConfirm" class="input2" style="width: 5em;" type="password" value=""><br>
                    <label style="font-size: 15px"><em>Recevez les derniers ajout de livre par mail ? (pas encore actif) : </em></label><input id="news" class="input2" type="radio" name="news" value="oui"><label>Oui</label><input value="non" id="news" class="input2" name="news" type="radio"><label>Non</label>
